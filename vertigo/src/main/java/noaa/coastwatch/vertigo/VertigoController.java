...
public class VertigoController {
  ...
  @FXML
  private CheckMenuItem viewControlMenuItem;
  ...
  @FXML
  private ToggleButton viewControlButton;
  ...
  /** The log console dialog. */
  private Dialog logDialog;

  /** The list of dialogs currently created. */
 ...
 /////////////////////////////////////////////////////////////////
  ...
  private void resizeWindow (
    ...
    if (newWidth > bounds.getWidth()) {
      ...
      LOGGER.warning ("Truncating new window width at " + (int) newWidth + " pixels");
    } // if

    if (newHeight > bounds.getHeight()) {
      newHeight = bounds.getHeight();
      LOGGER.warning ("Truncating new window height at " + (int) newHeight + " pixels");
    } // if
  ...
  // Updates the style sheet in the parent to the specified sheet.
  private void updateSheet (Parent node, String sheet) {

    var sheets = node.getStylesheets();
    sheets.set (0, sheet);
  ...
  private void handleDarkModeMenuItem (ActionEvent event) {
    ...
    var css = item.isSelected() ? "ui/vertigo_dark.css" : "ui/vertigo_light.css";
    var sheet = getClass().getResource (css).toExternalForm();

    updateSheet (viewPane.getScene().getRoot(), sheet);
    for (var dialog : dialogs) updateSheet (dialog.getDialogPane(), sheet);
  ...
  public void initialize () {
    ...
    // Set up binding for view control menu item and toggle button
    viewControlMenuItem.selectedProperty().bindBidirectional (viewControlButton.selectedProperty());
...
//    });
    ...
    // Create a list of dialogs to use later for updating the theme
...
//      areaListPaneController.setAreas (areaItems);
  ...
  /** Applies the stylesheets from the current root to the specified node. */
  ...
  private void showAbout() {
    ...
    if (aboutDialog == null) {
      ...
      var vertigoVersion = System.getProperty ("vertigo.version");
      var vertigoBuildNum = System.getProperty ("vertigo.build.number");
      var vertigoBuildDate = System.getProperty ("vertigo.build.date");
      var vmName = System.getProperty ("java.vm.name");
      var vmVersion = System.getProperty ("java.version");
      var vmDate = System.getProperty ("java.version.date");
      ...
      var mdText =
        ...
        ":--- | :---\n" +
        "**Build Date** | " + vertigoBuildDate + "\n" +
        "**Java Runtime** | " + vmName + " " + vmVersion + " (" + vmDate + ")\n" +
        ...
        " &nbsp; | Proj4J (github.com/locationtech/proj4j)\n";
  ...
  private void showLog() {
    ...
    if (logDialog == null) {
      logDialog = LoggingConsole.getInstance();
      applyStyle (logDialog.getDialogPane());
      logDialog.initModality (Modality.NONE);
      logDialog.initOwner (getStage());
      dialogs.add (logDialog);
    } // if
    if (!logDialog.isShowing()) showDialog (logDialog);
